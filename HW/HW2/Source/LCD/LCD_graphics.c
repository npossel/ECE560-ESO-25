...
#ifndef DRAW_LINE_RUNS_AS_RECTANGLES
	#define DRAW_LINE_RUNS_AS_RECTANGLES 0
#endif

#define STEP 2

#if 0 // don't compile if not testing
...
{
	PT_T p;
#if DRAW_LINE_RUNS_AS_RECTANGLES
	PT_T pe;
#endif
...
#if DRAW_LINE_RUNS_AS_RECTANGLES
		pe.Y = p.Y;
		pe.X = p.X + XAdvance*FinalPixelCount;
		LCD_Fill_Rectangle(&p, &pe, color);
#else
...
void Rotate_Points(PT_T * ipt, int num_points, PT_T * center, float roll_rad, PT_T * opt) {
	...
	int n;
	float sr, cr, sp, cp;
	// Update terminology per https://msl.cs.uiuc.edu/planning/node102.html ??
	...
	for (n=0; n<num_points; n++) {
		...
		opt[n].X = x*cr - y*sr + center->X;
		opt[n].Y = x*sr + y*cr + center->Y;
	}
}

void Rotate_Points_3_dev(PT_T * ipt, int num_points, PT_T * center, float roll_rad, float pitch_rad, PT_T * opt) {
	float x, y;
	int n;
	float sr, cr, sp, cp;
	// Update terminology per https://msl.cs.uiuc.edu/planning/node102.html ??
