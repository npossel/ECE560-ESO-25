...
/* Read touch screen. Returns 1 if touched, and updates position. 
	Else returns 0 leaving position unchanged. */
...
#ifdef USE_ADC_SERVER
	yd=request_conversion(LCD_TS_YD_ADC_CHAN);
#else
		// Read inputs
	  ...
	  x = yd;
		...
		// Read inputs
#ifdef USE_ADC_SERVER
		xr=request_conversion(LCD_TS_XR_ADC_CHAN);
#else
		ADC0->CFG2 &= ~ADC_CFG2_MUXSEL_MASK;
