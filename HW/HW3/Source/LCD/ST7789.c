...
void LCD_Plot_Pixel(PT_T * pos, COLOR_T * color) {
	...
	if ((pos->X < 0) || (pos->X > LCD_WIDTH-1) ||
			(pos->Y < 0) || (pos->Y > LCD_HEIGHT-1))
		return;
...
void LCD_Fill_Rectangle(PT_T * p1, PT_T * p2, COLOR_T * color) {
	int32_t n;
	uint8_t b1, b2;
	int16_t c_min, c_max, r_min, r_max;
...
#if DEV_PACKED_PIXEL_COLOR_T
  #error "Packed Pixel Color support missing in function." 			 																										
#else																												
	// 16 bpp, 5-6-5. Assume color channel data is left-aligned
	b1 = (R & 0xf8) | ((G & 0xe0) >> 5);
