...
void Control_HBLED(void) {
	...
	if (g_scope_state == Armed) {			// SOLUTION
		sample_idx = 0; 									// SOLUTION
	//   Trigger on set point low-to-high past threshold
		if ((g_set_current_mA >= threshold_mA) && (prev_set_current_mA < threshold_mA)) // SOLUTION
			g_scope_state = Triggered; 		// SOLUTION
	} 																	// SOLUTION
	if (g_scope_state == Triggered) { // SOLUTION
		g_set_sample[sample_idx] = g_set_current_code; // SOLUTION
		g_meas_sample[sample_idx] = res; 	// SOLUTION
		sample_idx++; 										
	//   Start buffer display if we have a full buffer. 
		if (sample_idx >= SAM_BUF_SIZE) { 
			sample_idx = 0;
			g_scope_state = Full;					// SOLUTION
			DEBUG_START(DBG_PENDING_WVFM_POS); // visualize latency		// SOLUTION
		}																	
	}																		// SOLUTION
